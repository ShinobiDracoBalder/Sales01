namespace Sales01.Services
{
    ...
    public class DataService
    {
        ...
        private async Task OpenOrCreateDB()
        {
            ...
            await connection.CreateTableAsync<ProductRequest>().ConfigureAwait(false);
        }
        ...
        public async Task<List<ProductRequest>> GetAllProducts()
        {
            var query = await this.connection.QueryAsync<ProductRequest>("select * from [ProductRequest]");
            var array = query.ToArray();
            var list = array.Select(p => new ProductRequest
            {
        ...
        {
            var query = await this.connection.QueryAsync<ProductRequest>("delete from [ProductRequest]");
        }
    }
}
